...
import { HeartPulse, ShieldCheck, Fingerprint } from 'lucide-react';
...
export default function HealthCard({ patient, publicUrlBase = '' }: Props) {
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto hp-card-gradient rounded-2xl p-5 text-white overflow-hidden hp-card-shadow">
      {/* decorative circles */}
      <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-white/10" />
      <div className="absolute -bottom-16 -left-12 w-48 h-48 rounded-full bg-sky-400/10" />

      <div className="relative">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-white/15 flex items-center justify-center ring-1 ring-white/25">
              <HeartPulse className="w-4.5 h-4.5" strokeWidth={2.2} />
            </div>
            <div>
        ...
        <div className="mt-4 pt-3 border-t border-white/15 flex items-center gap-1.5 text-[10px] text-sky-100/70">
          <ShieldCheck className="w-3 h-3" />
          <span>Scan QR to view verified medical history</span>
        </div>
      </div>
    </div>
  );
